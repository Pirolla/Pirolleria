	...
	$$ = $.fn.pirolleria = function(options) {
		...
		$.getJSON(_o.json, function(data) {
			$.getJSON(data.itemList, function(itemList) {
				...
				//       As JavaScript parses the doc form top to bottom and I think that this
				//       function should be the first one I don't know what to do
				$.each(itemList.items, function(i, item) {
					...
					var _thumb = $(document.createElement('img')).attr('src',_src).addClass('thumb').appendTo(_div);
