	...
	**/
	...
	$$.activate = function(_thumb) { 
		...
		if (_thumb) {
			...
			// if there is no div, create it and append to pirolleria, else, just show it
			var _div = $(document.createElement('div')).addClass('group');
			$('.pirolleria').append(_div);
			
			// hide the container div (better performace for DOM manipulation) and iterate through the images
			// I've heard discussions about visibility and display here - gotta research what is best practice
			_div.css('display', 'none');
			
			$.getJSON(_thumb.json, function(data) {
				$.getJSON(data.itemList, function(itemList) {
					
					// fetch the JSON attributes
					var _title			= data.title;
					var _description	= data.description;
					var _published		= data.published;
					var _author			= data.author;
					
					// maybe we should go with the description alone... fixing an h1 here doesn't sound right
					_div.append("<h1>" + _title + "</h1>");
					_div.append("<p>" + _description + "</p>");
					...
					$.each(itemList.items, function(i, item) {
						...
						var _shortDesc	= item.description;
